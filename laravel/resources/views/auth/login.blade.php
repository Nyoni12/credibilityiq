...
<div>
    ...
    <form method="POST" action="{{ route('login.post') }}" class="space-y-4"
          x-data="{ email: '{{ old('email') }}', showPwd: false }">
        @csrf
        ...
        <div class="flex items-center justify-between">
            ...
            {{-- Only visible once the user has typed an email address --}}
            <a x-show="email.length > 0"
               x-cloak
               :href="'/forgot-password?email=' + encodeURIComponent(email)"
               class="text-xs text-brand-600 hover:text-brand-800 hover:underline transition-colors">
                Forgot password?
            </a>
        </div>
